#!/usr/bin/env python3
"""Exact structural, mutation, artifact, and authority tests for Specials saves."""
...
from savegame_specials import SpecialsParseError, parse_specials_section  # noqa: E402


PREFIX = b"PRE-SPECIALS"
FOLLOWING_OWNER = b"WONDERS-SENTINEL" * 32


def _special_empty(special: int = 10, o: int = 20, flags: int = -3, who: int = 2) -> bytes:
    return struct.pack("<ihhbb", 0, special, o, flags, who)
...
def _special_spells(
    *,
    ...
    return (
        ...
        + struct.pack("<hhbb", special, o, special_flags, who)
    )
...
class SpecialsParserTests(unittest.TestCase):
    def test_complete_eight_owner_sparse_history_and_exact_wonders_boundary(self) -> None:
        data, offset, following_owner = _synthetic()
        parsed = parse_specials_section(data, offset)
        ...
        self.assertEqual(
            (empty.special, empty.o, empty.special_flags, empty.who, empty.size),
            (10, 20, -3, 2, 10),
        )
        ...
        self.assertEqual(
            (active.special, active.o, active.special_flags, active.who, active.size),
            (-30, 40, -7, 6, 41),
        )
    ...
    def test_every_owned_byte_mutation_is_killed_or_changes_receipt(self) -> None:
        ...
        baseline = parse_specials_section(data, offset)
        for relative in range(baseline.size):
            with self.subTest(relative=relative):
                damaged = bytearray(data)
                damaged[offset + relative] ^= 1
                try:
                    changed = parse_specials_section(damaged, offset)
                except SpecialsParseError:
                    continue
    ...
    def test_every_truncation_is_rejected(self) -> None:
        data, offset, _ = _synthetic()
        end = parse_specials_section(data, offset).end
        for retained in range(end - offset):
            with self.subTest(retained=retained):
                with self.assertRaises(SpecialsParseError):
                    parse_specials_section(data[: offset + retained], offset)

    def test_following_wonders_owner_mutation_is_excluded(self) -> None:
        data, offset, _ = _synthetic()
        baseline = parse_specials_section(data, offset)
        damaged = bytearray(data)
        damaged[baseline.end] ^= 0xFF
        self.assertEqual(parse_specials_section(damaged, offset), baseline)
    ...
    def test_outer_and_nested_structural_invariants_fail_closed(self) -> None:
        ...
        for name, (where, replacement) in cases.items():
            with self.subTest(name=name):
                ...
                with self.assertRaises(SpecialsParseError):
                    parse_specials_section(damaged, offset)

    def test_valid_histories_and_signed_special_scalars_are_preserved(self) -> None:
        baseline = parse_specials_section(_section(), 0)
        changed = parse_specials_section(
            _section(
                capacity=99,
                increment=-17,
                flags=0x25,
        ...
        self.assertEqual(
            (owner.slots[0].special_flags, owner.slots[2].special_flags), (-3, -7)
        )

    def test_active_spell_rows_cross_check_independent_sim_authority(self) -> None:
        data = _section()
        parsed = parse_specials_section(data, 0)
    ...
    def test_pdb_layout_and_executable_bodies_are_frozen(self) -> None:
        ...
        self.assertEqual(classes["Specials"]["size"], 232)
        self.assertEqual(classes["PtrArray<Special>"]["size"], 28)
        self.assertEqual(classes["Special"]["size"], 48)
        self.assertEqual(classes["SpecialData"]["size"], 48)
        ...
        spans = (
            ...
            (0x00740030, 42, "e50964bf5320a12417078c09184e40d339feb9d4112fb54f7807bc7c9953f5ee"),
            ...
            (0x005A2B61, 22, "f9673c8b49ceedaa13a4236242b3e72110547eaeb969553f6a7e0e889e9bcd22"),
    ...
    def test_fresh_svx_chains_to_exact_wonders_boundary_without_replay_join(self) -> None:
        root = pathlib.Path(__file__).resolve().parents[2]
        ...
        self.assertEqual(herds.end, 0x27073)
        parsed = parse_specials_section(save_plain, herds.end)
        self.assertEqual(
            (parsed.offset, parsed.end, parsed.size), (0x27073, 0x27094, 33)
        )
        self.assertEqual(parsed.tag, 0)
        ...
        self.assertEqual(
            hashlib.sha256(save_plain[parsed.end : parsed.end + 33]).hexdigest(),
            "7f9c9e31ac8256ca2f258583df262dbc7d6f68f2a03043d5c99a4ae5a7396ce9",
        )

        changed_next = bytearray(save_plain)
        changed_next[parsed.end] ^= 1
        self.assertEqual(parse_specials_section(changed_next, herds.end), parsed)
