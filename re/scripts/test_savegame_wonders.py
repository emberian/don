#!/usr/bin/env python3
"""Exact structural, mutation, artifact, and authority tests for Wonders saves."""
...
from savegame_wonders import WondersParseError, parse_wonders_section  # noqa: E402


PREFIX = b"PRE-WONDERS"
FOLLOWING_OWNER = b"FORTS-SENTINEL" * 32


def _row(wonder: int, o: int, stamp: int, timer: int, flags: int, who: int) -> bytes:
    return struct.pack("<hhiiBB", wonder, o, stamp, timer, flags & 0xFF, who & 0xFF)


ROW0 = _row(3, 9, 100, 200, 0xFD, 2)
ROW2 = _row(-1, -2, -300, 400, 1, -7)
...
class WondersParserTests(unittest.TestCase):
    def test_complete_eight_owner_sparse_history_and_exact_forts_boundary(self) -> None:
        data, offset, following = _synthetic()
        parsed = parse_wonders_section(data, offset)
        ...
        self.assertEqual(
            (first.wonder, first.o, first.stamp, first.timer, first.wonder_flags, first.who, first.size),
            (3, 9, 100, 200, -3, 2, 14),
        )
        self.assertFalse(hole.present)
        self.assertEqual(hole.presence_offset, owner.presence_offset + 1)
        self.assertEqual(
            (last.wonder, last.o, last.stamp, last.timer, last.wonder_flags, last.who, last.size),
            (-1, -2, -300, 400, 1, -7, 14),
        )

    def test_every_owned_byte_mutation_is_killed_or_changes_receipt(self) -> None:
        data, offset, _ = _synthetic()
        baseline = parse_wonders_section(data, offset)
        for relative in range(baseline.size):
            with self.subTest(relative=relative):
                damaged = bytearray(data)
                damaged[offset + relative] ^= 1
                try:
                    changed = parse_wonders_section(damaged, offset)
                except WondersParseError:
                    continue
    ...
    def test_every_truncation_is_rejected(self) -> None:
        data, offset, _ = _synthetic()
        end = parse_wonders_section(data, offset).end
        for retained in range(end - offset):
            with self.subTest(retained=retained):
                with self.assertRaises(WondersParseError):
                    parse_wonders_section(data[: offset + retained], offset)

    def test_following_forts_mutation_is_excluded(self) -> None:
        data, offset, _ = _synthetic()
        baseline = parse_wonders_section(data, offset)
        damaged = bytearray(data)
        damaged[baseline.end] ^= 0xFF
        self.assertEqual(parse_wonders_section(damaged, offset), baseline)
    ...
    def test_structural_invariants_fail_closed(self) -> None:
        ...
        for name, (where, replacement) in cases.items():
            with self.subTest(name=name):
                ...
                with self.assertRaises(WondersParseError):
                    parse_wonders_section(damaged, offset)

    def test_valid_history_and_exact_14_byte_rows_are_preserved(self) -> None:
        baseline = parse_wonders_section(_section(), 0)
        changed = parse_wonders_section(_section(99, -17, 0x25), 0)
        ...
        self.assertEqual(owner.slots[0].wonder_flags, -3)
        self.assertEqual(owner.slots[1].size, 0)

    def test_rows_cross_check_independent_canonical_wonder_authority(self) -> None:
        data = _section()
        parsed = parse_wonders_section(data, 0)
        first = parsed.owners[0].slots[0]
        self.assertEqual(data[first.offset : first.end], ROW0)
        self.assertEqual(first.size, 14)
        root = pathlib.Path(__file__).resolve().parents[2]
        authority = (root / "crates/don-sim/src/systems/wonders.rs").read_text()
        self.assertIn("The checksummed `WonderData` prefix (PDB size `0x10`).", authority)
        self.assertIn("pub struct WonderRecord {", authority)
        for field in (
            "pub wonder: i16,", "pub o: i16,", "pub stamp: i32,", "pub timer: i32,",
            "pub wonder_flags: u8,", "pub who: i8,",
        ):
            self.assertIn(field, authority)
        self.assertIn("six fields below are the simulation data walked", authority)
        self.assertEqual(first.wonder_flags & 0xFF, 0xFD)
    ...
    def test_pdb_layout_and_executable_bodies_are_frozen(self) -> None:
        ...
        self.assertEqual(classes["Wonders"]["size"], 232)
        self.assertEqual(classes["PtrArray<Wonder>"]["size"], 28)
        self.assertEqual(classes["Wonder"]["size"], 24)
        self.assertEqual(classes["WonderData"]["size"], 16)
        spans = (
            (0x0073CA40, 952, "213c09131e26796781f824df428d536ee332a7b0fe1dbc094bf061482f5d64e0"),
            (0x0073C780, 23, "dedb725c76e0c9fedf0879c252f8b738de57e8db5a1be0eba3e68082a14aa4ad"),
            (0x005A2B71, 22, "840ba7593fb85090cc26c5fb604e1bd4d7ac269c970289eef39c2959ab295994"),
    ...
    def test_fresh_svx_chains_to_exact_forts_boundary_without_replay_join(self) -> None:
        root = pathlib.Path(__file__).resolve().parents[2]
        ...
        self.assertEqual(specials.end, 0x27094)
        parsed = parse_wonders_section(save_plain, specials.end)
        self.assertEqual((parsed.offset, parsed.end, parsed.size), (0x27094, 0x270B5, 33))
        self.assertEqual(parsed.tag, 0)
        ...
        self.assertEqual(parse_wonders_section(changed, specials.end), parsed)
