#!/usr/bin/env python3
"""Exact structural, mutation, artifact, and authority tests for Docks saves."""
...
from savegame_docks import DocksParseError, parse_docks_section  # noqa: E402


PREFIX = b"PRE-DOCKS"
FOLLOWING_OWNER = b"OIL-WELLS-SENTINEL" * 32


def _row(dock: int, o: int, reg: int, flags: int, who: int, gull_o: int) -> bytes:
    # Save-mode stream order is +0..6, +8..10, then +6..8.
    return struct.pack("<hhhBBh", dock, o, reg, flags & 0xFF, who & 0xFF, gull_o)


ROW0 = _row(3, 9, 100, 0xFD, 2, 17)
ROW2 = _row(-1, -2, -300, 1, -7, -19)
...
class DocksParserTests(unittest.TestCase):
    def test_complete_eight_owner_sparse_history_and_exact_oil_wells_boundary(self) -> None:
        data, offset, following = _synthetic()
        parsed = parse_docks_section(data, offset)
        ...
        self.assertEqual(
            ...
            "f8a25e54511a849b51d99a8ff9d5e4cd3316b3a4fc33e5cead47e3efe72b1a74",
        )
    ...
    def test_every_truncation_is_rejected(self) -> None:
        data, offset, _ = _synthetic()
        end = parse_docks_section(data, offset).end
        for retained in range(end - offset):
            with self.subTest(retained=retained):
                with self.assertRaises(DocksParseError):
                    parse_docks_section(data[: offset + retained], offset)

    def test_following_oil_wells_mutation_is_excluded(self) -> None:
        data, offset, _ = _synthetic()
        baseline = parse_docks_section(data, offset)
        damaged = bytearray(data)
        damaged[baseline.end] ^= 0xFF
        self.assertEqual(parse_docks_section(damaged, offset), baseline)
    ...
    def test_structural_invariants_fail_closed(self) -> None:
        ...
        for name, (where, replacement) in cases.items():
            with self.subTest(name=name):
                ...
                with self.assertRaises(DocksParseError):
                    parse_docks_section(damaged, offset)

    def test_valid_history_and_exact_10_byte_save_rows_are_preserved(self) -> None:
        baseline = parse_docks_section(_section(), 0)
        changed = parse_docks_section(_section(99, -17, 0x25), 0)
        ...
        self.assertEqual(owner.slots[0].dock_flags, -3)
        self.assertEqual(owner.slots[1].size, 0)

    def test_save_mode_row_reorders_gull_after_flags_and_excludes_object_tail(self) -> None:
        data = _section()
        parsed = parse_docks_section(data, 0)
        first = parsed.owners[0].slots[0]
        self.assertEqual(data[first.offset : first.end], ROW0)
        self.assertEqual(first.size, 10)
        self.assertEqual(first.dock_flags & 0xFF, 0xFD)
        self.assertEqual(data[first.offset + 6 : first.offset + 8], b"\xfd\x02")
        self.assertEqual(data[first.offset + 8 : first.end], struct.pack("<h", 17))
    ...
    def test_pdb_layout_and_executable_bodies_are_frozen(self) -> None:
        ...
        self.assertEqual(classes["Docks"]["size"], 232)
        self.assertEqual(classes["PtrArray<Dock>"]["size"], 28)
        self.assertEqual(classes["Dock"]["size"], 20)
        self.assertEqual(classes["DockData"]["size"], 10)
        self.assertEqual(classes["DataWalk"]["flattened"][1]["name"], "checksum")
        self.assertEqual(classes["DataWalk"]["flattened"][1]["offset"], 8)
        spans = (
            (0x00741100, 1006, "4c07a13c608cef5eb988b7d86461677ec73bcc45f8cfaae81188875057c4fa87"),
            (0x00740C40, 72, "399b5458c49f0e4680e574ac8471101f95a5b568cec3a13ac6ca4d6fd7709375"),
            (0x005A2B91, 22, "092fd765f596649ba0430efe2f2a0b978fa24be02dbe09532c8b18dfacf1a459"),
            (0x0073F890, 952, "9359adf2f701afcbba18af51052e3243b4e234c772d083d3c55be5d8b25f5791"),
        )
    ...
    def test_fresh_svx_chains_to_exact_oil_wells_boundary_without_replay_join(self) -> None:
        root = pathlib.Path(__file__).resolve().parents[2]
        ...
        self.assertEqual(forts.end, 0x270D6)
        parsed = parse_docks_section(save_plain, forts.end)
        self.assertEqual((parsed.offset, parsed.end, parsed.size), (0x270D6, 0x270F7, 33))
        self.assertEqual(parsed.tag, 0)
        ...
        self.assertEqual(parse_docks_section(changed, forts.end), parsed)
