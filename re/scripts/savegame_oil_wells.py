#!/usr/bin/env python3
"""Parse the exact retail ``OilWells::walk_data`` generic-save image.

The helper starts at the OilWells tag, preserves all eight independent
``PtrArray<OilWell>`` histories and presence planes, decodes every exact
8-byte OilWell body, and stops before ``Supplies::walk_data``. PE control flow
...
TAG_OIL_WELLS = 0x00
TAG_STRING_TABLE_INDEX = 5071
OIL_WELL_OWNER_COUNT = 8
MAX_ARRAY_LENGTH = 1 << 20
DEFAULT_SCHEMA_PATH = pathlib.Path(__file__).resolve().parents[2] / "schema/pdb-types.json"


class OilWellsParseError(ValueError):
    """The stream or PDB layout contradicts the recovered OilWells traversal."""
...
@dataclasses.dataclass(frozen=True)
class OilWellsSection:
    offset: int
    end: int
    tag: int
    owners: tuple[OilWellOwner, ...]
    sha256: str
    layout_sha256: str
...
_EXPECTED_OIL_WELLS = (("lists", 0, 224, "PtrArray<OilWell>[8]"),)
_EXPECTED_PTR_ARRAY = (
    ("length", 4, 4, "int"),
    ("size", 8, 4, "int"),
...
def _load_layout_cached(path_text: str) -> _Layout:
    ...
    try:
        ...
        records = {
            "OilWells": classes["OilWells"],
            "PtrArray<OilWell>": classes["PtrArray<OilWell>"],
            "OilWell": classes["OilWell"],
            "OilWellData": classes["OilWellData"],
        }
    except (OSError, KeyError, TypeError, json.JSONDecodeError) as error:
        raise OilWellsParseError(
            f"cannot load OilWells PDB layout from {path}: {error}"
        ) from error
    expected = {
        "OilWells": (232, _EXPECTED_OIL_WELLS),
        "PtrArray<OilWell>": (28, _EXPECTED_PTR_ARRAY),
        "OilWell": (16, _EXPECTED_OIL_WELL),
        "OilWellData": (8, _EXPECTED_OIL_WELL),
    }
...
def _empty_slot(index: int, presence_offset: int) -> OilWellSlot:
    return OilWellSlot(index, False, presence_offset, None, None, None, None, None, None, None, None)


def _parse_slot(reader: _Reader, owner_index: int, index: int, presence_offset: int) -> OilWellSlot:
    start = reader.pos
    oil_well = reader.i16(f"OilWells[{owner_index}][{index}].oil_well")
    o = reader.i16(f"OilWells[{owner_index}][{index}].o")
    reg = reader.i16(f"OilWells[{owner_index}][{index}].reg")
    oil_well_flags = reader.i8(f"OilWells[{owner_index}][{index}].oil_well_flags")
    who = reader.i8(f"OilWells[{owner_index}][{index}].who")
    return OilWellSlot(
        index=index,
        present=True,
        ...
        o=o,
        reg=reg,
        oil_well_flags=oil_well_flags,
        who=who,
        sha256=hashlib.sha256(reader.data[start : reader.pos]).hexdigest(),
    )


def _parse_owner(reader: _Reader, index: int) -> OilWellOwner:
    start = reader.pos
    name = f"OilWells.owners[{index}]"
    length = reader.i32(f"{name}.length")
    if length < 0 or length > MAX_ARRAY_LENGTH:
        raise OilWellsParseError(f"{name} has invalid length {length}")
    if length == 0:
        return OilWellOwner(
    ...
    if any(value not in (0, 1) for value in presence):
        raise OilWellsParseError(f"{name} pointer-presence plane is not boolean")
...
def parse_oil_wells_section(
    ...
    *,
    ...
    """Parse one complete retail ``OilWells::walk_data`` at ``offset``.

    The returned ``end`` is the first byte owned by ``Supplies::walk_data``.
    """
    layout = _load_layout(schema_path)
    reader = _Reader(data, offset)
    tag = reader.u8("OilWells tag")
    if require_tag and tag != TAG_OIL_WELLS:
        raise OilWellsParseError(
            f"OilWells tag {tag:#04x} != {TAG_OIL_WELLS:#04x} at {offset:#x}"
        )
    owners = tuple(_parse_owner(reader, index) for index in range(OIL_WELL_OWNER_COUNT))
    return OilWellsSection(
        offset,
        reader.pos,
        tag,
        owners,
...
def _summary(section: OilWellsSection, path: pathlib.Path) -> str:
    present = sum(sum(owner.presence) for owner in section.owners)
    return "\n".join(
        (
            f"{path}: OilWells {section.offset:#x}..{section.end:#x} "
            f"({section.size} bytes) sha256={section.sha256}",
            f"  PDB-layout sha256={section.layout_sha256}",
            f"  owners=8 present_oil_wells={present}",
            f"  next owner begins at {section.end:#x} (Supplies::walk_data 0x0073ae90)",
        )
    )
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    section = parse_oil_wells_section(_load(args.file), args.offset, schema_path=args.schema)
