#!/usr/bin/env python3
"""Exact structural, mutation, artifact, and authority tests for OilWells saves."""
...
from savegame_oil_wells import OilWellsParseError, parse_oil_wells_section  # noqa: E402


PREFIX = b"PRE-OIL-WELLS"
FOLLOWING_OWNER = b"SUPPLIES-SENTINEL" * 32


def _row(oil_well: int, o: int, reg: int, flags: int, who: int) -> bytes:
    return struct.pack("<hhhBB", oil_well, o, reg, flags & 0xFF, who & 0xFF)
...
class OilWellsParserTests(unittest.TestCase):
    def test_complete_eight_owner_sparse_history_and_exact_supplies_boundary(self) -> None:
        data, offset, following = _synthetic()
        parsed = parse_oil_wells_section(data, offset)
    ...
    def test_every_truncation_is_rejected(self) -> None:
        data, offset, _ = _synthetic()
        end = parse_oil_wells_section(data, offset).end
        for retained in range(end - offset):
            with self.subTest(retained=retained):
                with self.assertRaises(OilWellsParseError):
                    parse_oil_wells_section(data[: offset + retained], offset)

    def test_following_supplies_mutation_is_excluded(self) -> None:
        data, offset, _ = _synthetic()
        baseline = parse_oil_wells_section(data, offset)
        damaged = bytearray(data)
        damaged[baseline.end] ^= 0xFF
        self.assertEqual(parse_oil_wells_section(damaged, offset), baseline)
    ...
    def test_structural_invariants_fail_closed(self) -> None:
        ...
        for name, (where, replacement) in cases.items():
            with self.subTest(name=name):
                ...
                with self.assertRaises(OilWellsParseError):
                    parse_oil_wells_section(damaged, offset)

    def test_valid_history_and_exact_8_byte_rows_are_preserved(self) -> None:
        baseline = parse_oil_wells_section(_section(), 0)
        changed = parse_oil_wells_section(_section(99, -17, 0x25), 0)
    ...
    def test_pdb_layout_and_executable_bodies_are_frozen(self) -> None:
        ...
        self.assertEqual(classes["OilWells"]["size"], 232)
        self.assertEqual(classes["PtrArray<OilWell>"]["size"], 28)
        self.assertEqual(classes["OilWell"]["size"], 16)
        self.assertEqual(classes["OilWellData"]["size"], 8)
        spans = (
            (0x0073F890, 952, "9359adf2f701afcbba18af51052e3243b4e234c772d083d3c55be5d8b25f5791"),
            (0x0073F650, 23, "eb786d977332486f8133bca2e1329153f6e075307402c1c95decd1fa6aec639b"),
            (0x005A2BA1, 22, "2c71321765d5c67918c92236a4b10f24897b8fad4ed359cbb996d3a4429b0b3d"),
    ...
    def test_fresh_svx_chains_to_exact_supplies_boundary_without_replay_join(self) -> None:
        root = pathlib.Path(__file__).resolve().parents[2]
        ...
        parsed = parse_oil_wells_section(save_plain, docks.end)
        self.assertEqual((parsed.offset, parsed.end, parsed.size), (0x270F7, 0x27118, 33))
        self.assertEqual(parsed.tag, 0)
        ...
        self.assertEqual(parse_oil_wells_section(changed, docks.end), parsed)
