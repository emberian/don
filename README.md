...
| **Retail differential oracle** | 23 registered i686 cases, 18,722,565 trials in the current record, with zero mismatches, skips, crashes, or errors. The harness runs shipped machine code and mutation-tests its comparisons. |
...
| **RL environment** | Batched Rust environment with Gymnasium, VectorEnv, and PettingZoo adapters; derived 82-opcode action taxonomy, parameter-level masks, zero-copy observations, native masked sampling, and deterministic parallel stepping. Unsupported dynamics are masked rather than accepted as no-ops; the current exhaustive action contract reports zero advertised illegal/no-effect actions. |
...
| **Live retail tools** | RoNtoy live economy coach and overlay; hash-bound injection, read-only state capture, passive network observation, and reversible main-thread command ingress. Five active-match STOP/rearm cycles restored the exact stock bytes with zero dropped events, and a redacted read-only snapshot proved bounded join/leave peering with a real retail host. Match launch and multiplayer turn/checksum/replay agreement remain open. |
| **BHS and mods** | Recovered source compiler, bytecode decoder, stack VM, builtin registry, fail-closed host boundary, aggregate execution, content overlays, mod discovery/path compatibility, and fidelity/strict behavior gates. Five body-bearing shipped-compiler fixtures are 5/5 byte-identical across code, constant pools, and script metadata. |
