...
| Opcode 48 Build unqueue transaction | `Build::action_unqueue` `0x00620280`, `Build::unqueue` `0x006207C0`, `LeaderData::get_first_library` `0x006DB6C0`, `Build::unpay_cost` `0x006206E0` | `don-sim/src/systems/build_action_unqueue.rs`; canonical host in `production_runtime.rs` | **C at best, instruction-derived; no retail execution** — complete PE/PDB/Capstone bodies and Rust real-command-path tests, not differential evidence |
...
| `Game::run` direct visibility refresh and frame-zero explored sharing | direct call `0x0058525D`; `GameDaemon::update_all_seen` tail `0x00732BD8..0x00732CF3` | `don-sim/src/systems/step12_visibility_runtime.rs`, mounted by `don-sim/src/tick.rs::Sim::game_run_update_all_seen` | **C, instruction-derived; no retail execution** — exact active-slot, mutual-diplomacy, ally-mask, option, subset, and intersecting-`seen2` branches; executable atomic/save-resume tests cover the bounded GameRun route; `Build::close` and `set_explored_show_buildings` remain outside |
| Scenario reveal points and paired visibility direct routes | `ScenarioFuncSet::add_reveal_point` `0x009E4750`; `clear_reveal_points` `0x009E4810`; `add_visibility` `0x009FC710`; `remove_visibility` `0x009FC7B0`; `GameDaemon::update_all_seen` block `0x00732A7D..0x00732BD8` | `don-sim/src/script_runtime.rs`, `don-sim/src/systems/step12_visibility_runtime.rs`, mounted by `don-sim/src/tick.rs::Sim::scenario_set_visibility` | **C, instruction-derived; no retail execution** — exact validation, zero-capacity array growth, checksum/save metadata, repeated-array-zero producer quirk, and atomic direct preflight/commit; executable BHS/checksum/save-resume tests, no retail differential claim |
