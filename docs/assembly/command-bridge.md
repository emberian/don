...
**(b) `PATROL` and `LAUNCH_PATROL` are not `MOVE_TO`.** `COVERAGE.md` §3.1 already flags
that `don-env::action::apply_unit` collapses `MOVE_TO`/`MOVE_NEAR`/`PATROL`/`LAUNCH_PATROL`
onto one `OrderIndex::MoveTo` arm. The bridge now supplies the measured answer rather than
the observation that it is wrong: opcode 10 → `Group::action_patrol` →
`Unit::add_patrol_order` → **`GROUP_PATROL` (22)**; opcode 11 →
`Group::action_launch_patrol` → `Unit::add_air_patrol_order` → **`AIR_PATROL` (17)**. The
test drives both through the real wire path and asserts the kinds.
