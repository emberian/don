...
**(b) `PATROL` and `LAUNCH_PATROL` are not `MOVE_TO`.** For `QUEUE_NEW`, the env now
...
branches, and exercises the env mask/application path with shipped type records. Execution
of orders 17 and 22 and `QUEUE_FIRST`/`QUEUE_LAST` insertion remain a separate readiness
blocker; the env reports the latter two as `accepted_no_effect` rather than silently
overwriting its one current-order slot.
