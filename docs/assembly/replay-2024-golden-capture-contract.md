...
## Minimal mandatory bundle

`setup_2024_golden_capture::GoldenCaptureManifest` is the source-of-truth capture topology for
the supported 2024 golden replay. Schema version 2 requires the unconditional Dutch
civilization-specific Market, Build `o=2001` type 436, before any starting Unit receiver. A
historical center-plus-seven manifest cannot validate.

The minimal bundle has 11 unique canonical DoNSave images:
...
`bind_captured_frame379_setup_entry` admits image 1 only after the completed `place_all` and
...
guessing an unresolved placement site.

`bind_captured_frame379_completed_init` admits each of images 2--8. The manifest requires exact
...
`bind_captured_frame2_post_tick` admits images 10 and 11. It verifies that the replay contains no
...
This contract does not create production capture bytes. The exact Great Lakes run still needs
the installed displacement assets and completed mode-5 world-generation owner before image 1 can
be captured lawfully. The seven native Unit receiver traces and the frame-zero/frame-one tick
images are also capture debt. Until all mandatory images and receipts exist, no golden replay or
corpus advancement is claimed.
