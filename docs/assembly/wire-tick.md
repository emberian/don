...
|--:|---|---|---|
| 8 | `Leaders::process_all` `0x006ED2A0` | recovered whole dispatcher; hostile scan; `leader_gather`; rare-mask dirty protocol; wall/unit stat-band traversals; resolved `is_active`/`is_captain` and base `Object::update_hits/update_los` stores; `process_elimination`; grace timers; taunt-table dispatch | building `Wall::update_hits/update_los` overrides; `Wall::update_construct_time`; direct `Unit::update_speed/update_armor`; `process_taunt` AI-chat body |
