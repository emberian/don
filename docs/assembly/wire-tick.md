...
|--:|---|---|---|
| 8 | `Leaders::process_all` `0x006ED2A0` | recovered whole dispatcher; hostile scan; `leader_gather`; rare-mask dirty protocol; wall/unit stat-band traversals; resolved `is_active`/`is_captain`, base `Object::update_hits/update_los`, full `Unit::update_speed`, `ObjectData::armor`, and `Unit::update_armor` propagation; `process_elimination`; grace timers; taunt-table dispatch | building `Wall::update_hits/update_los` overrides; `Wall::update_construct_time`; automatic speed/armor query population; `process_taunt` AI-chat body |
