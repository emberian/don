...
| `arena-construction-placement-model` | complete `blocked_site` transaction | *drift*: shipped Barracks has an identity-bearing live footprint claim; other type families and border invalidation remain absent | — |
| `arena-construction-lifecycle-model` | start/activate/reject/complete world transactions | *drift*: ResearchModel runs an identity-bearing Barracks transaction, but reswarm/animation and broad activation callbacks remain compact projections | — |
