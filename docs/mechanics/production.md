...
`crates/don-sim/src/systems/production.rs` (85 focused tests, all green).
...
| Queue completion effect routing | `Build::finished` `0x00628490`; `Build::train` `0x0062F9B0`; `Leader::gain_tech` `0x006DCB60` | `execute_finished_effect`, `execute_unit_completion`, `preflight_unit_placement`, `execute_unit_placement`, `execute_building_completion`, `FinishedEffectHost`, `TechState::gain` |
...
**Verification method.** 85 focused unit tests through the wired `don-sim` crate. They are behavioural
...
- **`Build::queue_up` `0x00620F40` (4,322 B) and `Build::train` `0x0062F9B0` (2,708 B)** —
  ...
  placement/rally tail. `preflight_unit_placement` now owns the recovered classifier at
  `0x0062FAC0..0x0063042E`: the missile/Helicopter masks, exact target-at-coordinate query,
  enemy and missile-defense branch, allied carry admission, aircraft count/limit branch,
  University Scholar handling, default garrison limit 10, and strict Aircraft Carrier
  routing. The host supplies only typed read-only object-graph facts; there is no opaque
  route callback. `execute_unit_placement` consumes that single plan and owns the mutation
  order: gather-point validation, live per-point order-list reads, first air-patrol versus
  waypoint append, either exact strafe form, `come_out(0)`, live post-release garrison
...
6. **Completion-effect host integration.** `execute_finished_effect` preserves retail's
   ...
   Placement object/order mutations, spell casting, the remainder of the 15,001-byte
   `gain_tech` body, and the Capitol hero train/upgrade remain mandatory host callbacks
   because they allocate or mutate world objects this module does not own. Placement route
   classification itself is executable here; its world reads are explicit typed facts.
...
gate. The current focused gate is **85 passed, 0 failed**. `production::adler32` already
