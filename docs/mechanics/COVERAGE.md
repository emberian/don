...
| 9 | `cities` | `check_cities` `0x00937600` | `City::walk_data` `0x00489220` | 110-byte POD/flags prefix plus dynamic `Array<CaravanLink>` | `tech_cities`, `don-replay::cities_runtime`, `don-replay::starting_city_unit_census` | partial; exact conditional Sim bridge and activation-time TData CITY masks on 3 corpus recordings; Unit census not mounted, terrain census stops at generated-content facts |
...
| 19 | **`Leader::process_event_frame`** | `0x006EC180` | **complete** | exact receipt-ordered rate/mood/battle mutations; one installed ordered outbox owns all reached JukeBox/Achieve calls while wall-clock audio remains outside deterministic state |
...
8. **`Leaders::process_all` step 8 derived stats** — the recovered 387-byte dispatcher now
