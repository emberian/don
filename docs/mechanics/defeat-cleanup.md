...
`0x006F9180`. The target is identified, but the 580-byte stop transaction still reaches
standing-Army, Group, and Unit order/path state not yet connected through one complete live
adapter. It is the next deterministic defeat boundary; presentation/network notifications
later in `Leader::defeat` remain intentionally separate. The other frozen object boundary
is the remainder of `Unit::close` after the current live death/corpse adapter: type-specific
counts, containment and Group teardown still need their own exact host facts.
