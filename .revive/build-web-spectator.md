...
YOUR LANE: web-spectator
WRITE YOUR REPORT TO: /Users/ember/dev/don/docs/tracks/web-spectator.md

BUILD THE WEB SPECTATOR: view a simulation or a cluster of simulations, spectate, and lay the
path to drop-in play. Highest-performance browser techniques, not a comfortable framework.

Prior design work may exist at docs/tracks/web-frontend.md and web/ — read it, keep what is
good, and GO FURTHER. The goal this round is something that actually renders our real sim.

1. Compile crates/don-sim to WASM and let JavaScript view its linear memory directly. The
   world is structure-of-arrays with fixed capacities, so position columns become typed-array
   views with ZERO COPY. Never serialise entities to JSON.
2. Render with WebGPU (WebGL2 fallback): instanced draws with the per-instance buffer written
   straight from the position columns. Thousands of units at 60 fps is the bar.
3. CLUSTER VIEW: many worlds at once as a grid of viewports, plus an aggregate statistics view
   for when individual worlds are too small to read. This is what makes watching a training
   run possible.
4. Run the sim in a worker with SharedArrayBuffer so the main thread only renders; document
   the COOP/COEP headers required. OffscreenCanvas where supported.
5. MEASURE and report fps against entity count and world count on this machine, with method.
   A spectator that holds 60 fps at 100 units and dies at 10,000 is not done.
6. Implement enough of drop-in play to prove the path: a viewer selecting units and issuing an
   order, encoded into the same action space crates/don-env uses.

Ship something a human can open and watch.
...
Files it already wrote or edited (9):
  /Users/ember/dev/don/web/tools/pack-gamedata.mjs
  /Users/ember/dev/don/web/tools/gen-wire.mjs
  /Users/ember/dev/don/web/wasm/src/gamedata.rs
  /Users/ember/dev/don/web/wasm/src/real.rs
  /Users/ember/dev/don/web/wasm/src/lib.rs
  /Users/ember/dev/don/web/wasm/src/bin/digest.rs
  /Users/ember/dev/don/web/public/js/wasm.js
  /Users/ember/dev/don/web/public/js/render.worker.js
  /Users/ember/dev/don/web/public/js/sim.worker.js

Commands it ran (25 distinct, first 15):
  $ ls -la /Users/ember/dev/don/ && echo "--- web ---" && ls -la /Users/em
  $ cat web/README.md && echo "=== build.sh ===" && cat web/build.sh && ec
  $ ls docs/tracks/ && echo "=== don-sim ===" && find crates/don-sim -type
  $ wc -l docs/tracks/web-frontend.md web/public/js/*.js web/public/index.
  $ ls crates/don-ai/src crates/don-net/src && cat crates/don-sim/src/lib.
  $ sed -n 1,80p crates/don-ai/src/lib.rs && echo "=== api.rs head ===" &&
  $ grep -n '^pub fn\|^pub struct\|^pub const\|^    pub [a-z_]*:\|^impl \|
  $ ls -la schema/live/ && head -c 400 schema/command-wire.json && echo &&
  $ head -2 schema/live/live-tables-unit.tsv | cut -c1-2000 && echo "--- r
  $ sed -n 380,520p crates/don-sim/src/mechanics.rs
  $ sed -n 140,380p crates/don-sim/src/mechanics.rs
  $ head -40 schema/live/rules-block-pid14644.txt && echo "=== grep flank_
  $ python3 - <<'EOF'
  $ sed -n 509,760p crates/don-sim/src/mechanics.rs
  $ sed -n 760,880p crates/don-sim/src/mechanics.rs
