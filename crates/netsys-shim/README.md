...
The generation-7 PE32 runtime smoke now passes three times under Wine Devel 11.10's
new-WoW64 runtime. The original disposable-prefix attempt timed out while
`wineboot` was still installing fonts and fake DLLs, then left only the x64
`system32` layer when `wineserver` terminated before installing i386
...
generation-7 extension crossed Wine Devel 11.10 three times, but it is still not
retail live-load evidence. Only after explicit authorization may a future
exercise perform a load-only run with `DON_NET_LOAD_ONLY=1` and inspect the
flushed trace before enabling transport.
...
On macOS with Wine Devel 11.10, run the exact frozen generation-7 artifacts
through a fresh, pre-seeded new-WoW64 prefix and retain a receipt:

```sh
crates/netsys-shim/run-gen7-wine-smoke.sh \
  /tmp/don-gen7-artifacts.bUTKzW/CrossplayNetLib.dll \
  /tmp/don-gen7-artifacts.bUTKzW/netsys-load-smoke.exe \
  /tmp/don-gen7-smoke-evidence
```

The runner refuses non-generation-7 hashes and existing evidence files. It
removes only its own `mktemp` prefix unless `DON_KEEP_WINE_PREFIX=1` is set.
