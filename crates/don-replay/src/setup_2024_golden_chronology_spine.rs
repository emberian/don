...
use crate::setup_2024_frame379::{Frame379SetupEntryReceipt, REPLAY_FILE_SHA256, SETUP_CALLS};
...
pub fn compose_golden_chronology_spine(
    ...
    if !blocked_location.validates()
        ...
        || blocked_location.next_child.call_va == 0
        || blocked_location.next_child.callee_va == 0
    {
    ...
    let market_evidence_digest =
        golden_market_evidence_digest(market, blocked_location_receipt_sha256);
    let first_missing_authority =
        GoldenChronologyFirstMissingAuthority::MarketScoringChild(GoldenMarketExecutionResidual {
            child_call_va: blocked_location.next_child.call_va,
            child_callee_va: blocked_location.next_child.callee_va,
    ...
    match &receipt.first_missing_authority {
        GoldenChronologyFirstMissingAuthority::MarketScoringChild(residual)
            if residual.blocked_location.validates()
                ...
                && residual.child_call_va == residual.blocked_location.next_child.call_va
                && residual.child_callee_va == residual.blocked_location.next_child.callee_va
