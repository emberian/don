...
//! Replay reconstruction now advances the same map through receipt-bound continent,
//! post-continent and fertility transitions; unchanged bytes remain unknown.
//!
...
impl WorldOwnerLedger {
    ...
    pub fn advance_exact_port(
        ...
        // ownership section-by-section, but only when the proof admits the
        // growing section and no byte in that section was already owned. The
        // latter guard avoids pretending that section-local offsets retain
        // their meaning across an insertion into a partially owned stream.
        let mut rebased_owners = Vec::with_capacity(after.image.len());
        for section in WorldSection::all() {
            let before_window = self.snapshot.section(section);
            let after_window = after.section(section);
            let before_owners = &self.owners[before_window.start..before_window.end];
            if before_window.len() != after_window.len() {
                if !proof.allowed_sections.contains(section) {
                    return Err(WorldOwnerError::ForbiddenSectionShapeMutation {
                        section,
                        before_bytes: before_window.len(),
                        after_bytes: after_window.len(),
                    });
                }
                if after_window.len() < before_window.len() {
                    return Err(WorldOwnerError::SectionWalkShrank {
                        section,
                        before_bytes: before_window.len(),
                        after_bytes: after_window.len(),
                    });
                }
                if before_owners.iter().any(Option::is_some) {
                    return Err(WorldOwnerError::OwnedSectionShapeMutation { section });
                }
                rebased_owners.resize(rebased_owners.len() + after_window.len(), None);
            } else {
                rebased_owners.extend_from_slice(before_owners);
            }
        }
        ...
        for section in WorldSection::all() {
            ...
            while cursor < next.len() {
                if before.get(cursor) == next.get(cursor) {
                    cursor += 1;
                    continue;
                }
                if !proof.allowed_sections.contains(section) {
                ...
                let start = cursor;
                while cursor < next.len() && before.get(cursor) != next.get(cursor) {
                    cursor += 1;
                }
        ...
        for range in &changed {
            ...
            for owner in &mut rebased_owners
