...
#[test]
fn same_kind_player_stream_executes_again_after_the_common_dispatch_edge() {
    let mut world = world();
    let mut groups = TerrainGroups {
        groups: vec![group(6, 100, 0), group(4, 0, 0), group(6, 100, 0)],
        ..TerrainGroups::default()
    };
    let mut mountains = Mountains::default();
    ...
    assert_eq!(preview.completed_placement_groups, [0, 2]);
    assert_eq!(
        preview
    ...
    assert!(preview
        .player_group_dispatches
        .iter()
        .all(|dispatch| dispatch.outcome
            == don_sim::systems::terrain_groups::PlayerPatternGroupOutcome::Complete));
    assert_eq!(
        preview
            .player_group_dispatches
            .iter()
            .map(|dispatch| dispatch.calls.len())
            .collect::<Vec<_>>(),
        [1, 1]
    );
    assert_eq!(preview.player_group_prefix.as_ref().unwrap().len(), 2);
    assert_eq!(
        host_events,
        [
            PlaceAllHostEvent::NetDaemonProcessAll { group_index: 0 },
