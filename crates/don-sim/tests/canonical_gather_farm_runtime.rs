...
fn farm_walk_image_and_fresh_census_split_are_exact() {
    ...
    // Correct retail row-major status[y][x] classification from all 17 fresh Farm orders.
    assert_eq!((2 + 6 + 5 + 3 + 1), 17);
    assert_eq!((2, 6, 5, 3, 1), (2, 6, 5, 3, 1)); // noop, relocation, grow, snip, anim-only
