...
use don_sim::systems::armies::ST_MUSTERING;
...
fn reached_army_authority_is_unavailable_and_rolls_back_every_owner() {
    with_large_stack(|| {
        ...
        sim.armies.lists[2][3].status = ST_MUSTERING;
...
#[test]
fn active_winner_victory_then_empty_army_retirement_resumes_identically() {
    with_large_stack(|| {
        let mut uninterrupted = configured_alliance_victory_sim();
        let army = &mut uninterrupted.armies.lists[2][3];
        army.valid = 1;
        ...
        army.target_o = 77;
        army.list[0] = 123;
        let checkpoint = save_sim(&uninterrupted).expect("empty live Army input is savable");
        let mut resumed = load_sim(&checkpoint).expect("empty live Army input reloads");
        resumed.replace_diplomacy_authority(complete_facts());
        ...
        assert_eq!(army_receipt.after.city, 0);
        assert_eq!(army_receipt.after.target_o, 77);
        assert_eq!(army_receipt.after.list[0], 123);
        assert_ne!(
            resumed.vic_leaders.slots[2].leader_flags & leader_flag::WON,
            0
        );
...
#[test]
fn active_winner_empty_bit4_drop_difficulty_marching_resumes_identically() {
    with_large_stack(|| {
        let mut uninterrupted = configured_alliance_victory_sim();
        let army = &mut uninterrupted.armies.lists[2][3];
        army.valid = 1;
        ...
        uninterrupted.vic_match.set_sem(game_sem::NET_OR_RECORDING);
        uninterrupted.vic_leaders.slots[2].strategy[5] = 4;
        // This saved value is the exact value written by DropControl state 3. It fails the
        // signed `< 3` defending test and falls through to marching.
        uninterrupted.vic_leaders.slots[2].multi_diff = 3;
        let checkpoint = save_sim(&uninterrupted).expect("drop difficulty is savable");
        let mut resumed = load_sim(&checkpoint).expect("drop difficulty reloads");
        assert!(resumed.vic_match.sem(game_sem::NET_OR_RECORDING));
        assert_eq!(resumed.vic_leaders.slots[2].strategy[5], 4);
        assert_eq!(resumed.vic_leaders.slots[2].multi_diff, 3);
        resumed.replace_diplomacy_authority(complete_facts());
        ...
        assert_eq!(army_receipt.muster_difficulty.unwrap().multi_diff, 3);
        assert_eq!(army_receipt.after.human_frame, 0);
        assert_eq!(army_receipt.after.valid, 0);
        assert_eq!(army_receipt.after.status, 0);
        assert_eq!(army_receipt.after.city, -1);
        ...
        assert_eq!(resumed.channel_digest(), uninterrupted.channel_digest());
        let reloaded = load_sim(&save_sim(&resumed).unwrap()).expect("marching result reloads");
        assert_eq!(save_sim(&reloaded).unwrap(), save_sim(&resumed).unwrap());
    });
}
