...
use don_sim::systems::air_group_action_transaction::{
    AirGroupActionPlan, AirTransactionStatus, CanonicalObjectBand, CommandPackagePosition,
};
...
const RETAIL_BUILD_UNIT_FLIGHT_IDENTITY: AirReplayPackageIdentity = AirReplayPackageIdentity {
    game_frame: 39_338,
    package_serial: 39_550,
    play: 0,
};
const RETAIL_UNIT_UNIT_FLIGHT_IDENTITY: AirReplayPackageIdentity = AirReplayPackageIdentity {
    game_frame: 90_353,
    package_serial: 15_606,
    play: 0,
};
const RETAIL_PATROL_UNIT_FLIGHT_IDENTITY: AirReplayPackageIdentity = AirReplayPackageIdentity {
    game_frame: 111_040,
...
const RETAIL_RETURN_FLIGHT_IDENTITY: AirReplayPackageIdentity = AirReplayPackageIdentity {
    game_frame: 27_217,
    package_serial: 4_537,
    play: 0,
};
...
fn retail_build_unit_flight_commands() -> Vec<Vec<u8>> {
    [
        "0001001008",
...
fn retail_unit_unit_flight_commands() -> Vec<Vec<u8>> {
    [
        "4f0008000000000000",
...
fn retail_patrol_unit_flight_commands() -> Vec<Vec<u8>> {
    [
        "4f0008000000000000",
        "000001",
...
fn retail_return_flight_commands() -> Vec<Vec<u8>> {
    [
        "4f0008000000000000",
        "00020109000b00",
...
fn build_unit_flight_fixture() -> (Sim, Handle) {
    let mut sim = build_flight_fixture();
    sim.world.frame = RETAIL_BUILD_UNIT_FLIGHT_IDENTITY.game_frame;
    sim.vic_match.frame = RETAIL_BUILD_UNIT_FLIGHT_IDENTITY.game_frame;
    let mut target = None;
    for o in 0..=159i16 {
        let handle = sim
            .spawn_unit(3, 91, 48_000 + i32::from(o), 37_000, 4)
            .unwrap();
        assert_eq!(sim.world.units.o()[sim.world.row_of(handle).unwrap()], o);
        target = Some(handle);
    }
...
fn install_unit_target_strafe(sim: &mut Sim, actor: Handle, target: Handle) {
    ...
    let target_o = i32::from(sim.world.units.o()[target_row]);
    let target_who = i32::from(sim.world.units.get_who(target_row));
    let target_uid = sim.world.units.get_uid(target_row);
    let target_position = (
        sim.world.units.x_internal()[target_row],
        sim.world.units.y_internal()[target_row],
    );
    ...
    let payload = StrafeOrder {
        ...
        xx: target_position.0,
        yy: target_position.1,
        ..Default::default()
    };
    sim.world
...
fn install_unit_flight_authority(
    sim: &mut Sim,
    planes: &[Handle],
    fresh_flight_target: Option<(i32, i32)>,
) {
    sim.replace_group_move_authority(GroupMoveAuthority {
        revision: 0x15606,
        composition_digest: [0x06; 32],
        destination_is_water: false,
        force_formation_facing_zero: false,
        members: planes.iter().copied().map(move_member).collect(),
    });
...
fn unit_unit_flight_fixture() -> (Sim, Vec<Handle>, Handle) {
    const SELECTED: [usize; 13] = [187, 194, 243, 77, 83, 196, 200, 202, 255, 256, 160, 124, 47];
    let mut sim = Sim::new(0x1c_15606, 256);
    let mut players = PlayerTable::new();
    players.seat(0, 1, 1, 0);
    sim.players = Some(players);
    sim.world.frame = RETAIL_UNIT_UNIT_FLIGHT_IDENTITY.game_frame;
    sim.vic_match.frame = RETAIL_UNIT_UNIT_FLIGHT_IDENTITY.game_frame;

    let owner_units: Vec<_> = (0..=256)
        .map(|o| {
            sim.spawn_unit(1, PLANE_TYPE, 21_000 + o * 16, 24_000, 4)
                .unwrap()
        })
        .collect();
    let mut target_units = Vec::new();
    for o in 0..=182 {
        target_units.push(sim.spawn_unit(3, 91, 49_000 + o * 8, 43_000, 4).unwrap());
    }
    let initial_target = target_units[0];
    let target = target_units[182];
    let planes = SELECTED.map(|o| owner_units[o]).to_vec();
    for &plane in &planes {
        install_unit_target_strafe(&mut sim, plane, initial_target);
    }
    install_unit_flight_authority(&mut sim, &planes, None);
    (sim, planes, target)
}

fn return_flight_fixture() -> (Sim, [Handle; 2], usize, u16) {
    let mut sim = Sim::new(0x1c_4537, 64);
    let mut players = PlayerTable::new();
    players.seat(0, 1, 1, 0);
    sim.players = Some(players);
    sim.world.frame = RETAIL_RETURN_FLIGHT_IDENTITY.game_frame;
    sim.vic_match.frame = RETAIL_RETURN_FLIGHT_IDENTITY.game_frame;

    let units: Vec<_> = (0..=11)
        .map(|o| {
            sim.spawn_unit(1, PLANE_TYPE, 20_000 + o * 16, 24_000, 4)
                .unwrap()
        })
        .collect();
    for local in 0..=8u16 {
    ...
    let planes = [units[9], units[11]];
    let initial_uid = sim.builds[0].uid;
    for &plane in &planes {
        let row = sim.world.row_of(plane).unwrap();
        sim.world.units.group_mut()[row] = -1;
        sim.world.units.o_down_mut()[row] = -1;
        sim.world.orders_mut(row).replace(
            Order::strafe(
                StrafeOrder {
                    target_o: 2_000,
                    target_who: 1,
    ...
    sim.replace_air_group_authority(AirGroupRuntimeAuthority {
        ...
        units: planes
            ...
            .map(|handle| AirGroupUnitAuthority {
                ...
                is_helicopter: false,
                fresh_flight_target: None,
                return_flight_target: Some((1, 2_008)),
                is_nuclear_missile: false,
            })
            .collect(),
        builds: vec![BuildSelectionAuthority {
    ...
    (sim, planes, 8, initial_uid)
}

fn patrol_unit_flight_fixture() -> (Sim, Vec<Handle>, Handle) {
    const SELECTED: [usize; 9] = [12, 38, 40, 41, 42, 43, 44, 45, 46];
    let mut sim = Sim::new(0x1c_6090, 256);
    let mut players = PlayerTable::new();
    players.seat(0, 1, 1, 0);
    sim.players = Some(players);
    sim.world.frame = RETAIL_PATROL_UNIT_FLIGHT_IDENTITY.game_frame;
    sim.vic_match.frame = RETAIL_PATROL_UNIT_FLIGHT_IDENTITY.game_frame;

    let owner_units: Vec<_> = (0..=46)
        .map(|o| {
            sim.spawn_unit(1, PLANE_TYPE, 31_000 + o * 16, 35_000, 4)
                .unwrap()
        })
        .collect();
    let mut target_units = Vec::new();
    for o in 0..=18 {
        target_units.push(sim.spawn_unit(4, 91, 51_000 + o * 8, 47_000, 4).unwrap());
    }
    let initial_target = target_units[0];
    let target = target_units[18];
    let planes = SELECTED.map(|o| owner_units[o]).to_vec();
    for &plane in &planes {
        install_unit_target_strafe(&mut sim, plane, initial_target);
    }
    ...
    cache[0] = planes
        ...
        .collect();
    sim.command_package_state = CommandPackageState::from_saved_selections(cache).unwrap();
    install_unit_flight_authority(&mut sim, &planes, Some((4, 18)));
    (sim, planes, target)
}
...
#[test]
fn exact_retail_explicit_airbase_flight_to_unit_is_no_action_and_save_resumable() {
    let (mut sim, target) = build_unit_flight_fixture();
    let commands = retail_build_unit_flight_commands();
    let target_row = sim.world.row_of(target).unwrap();
    let target_orders = sim.world.orders(target_row).clone();
    let target_uid = sim.world.units.get_uid(target_row);
    let target_position = (
        sim.world.units.x_internal()[target_row],
        sim.world.units.y_internal()[target_row],
    ...
    let receipt = sim
        .process_air_replay_batch(RETAIL_BUILD_UNIT_FLIGHT_IDENTITY, &commands)
        .unwrap();
    assert!(receipt.validates());
    assert_eq!(receipt.command_image, commands);
    assert_eq!(receipt.flight_no_action.len(), 1);
    ...
    assert!(receipt.air.is_empty());
    let flight = &receipt.flight_no_action[0];
    assert_eq!(flight.target.band, CanonicalObjectBand::Unit);
    assert_eq!(flight.target.address(), (3, 159));
    assert!(matches!(
        flight.target.generation,
        don_sim::systems::air_group_action_transaction::CanonicalObjectGeneration::Unit {
            id,
            generation,
        } if (id, generation) == (target.id, target.generation)
    ));
    assert_eq!(flight.target_uid, target_uid);
    assert_eq!(flight.target_position, target_position);
    assert_eq!(flight.selected_airbases.len(), 1);
    assert_eq!(
        (
            flight.selected_airbases[0].who,
    ...
    assert!(flight.contained_non_missiles.is_empty());
    assert_eq!(sim.world.orders(target_row), &target_orders);
    assert_eq!(sim.world.random.state(), before_rng);
    assert_eq!(
        sim.builds.iter().map(BuildData::image).collect::<Vec<_>>(),
        before_builds,
...
#[test]
fn changed_no_action_unit_target_rejects_before_explicit_airbase_selection_publication() {
    let (mut sim, target) = build_unit_flight_fixture();
    let commands = retail_build_unit_flight_commands();
    ...
    let prepared = prepare_canonical_air_replay_batch(
        ...
        &players,
        map_tiles,
        RETAIL_BUILD_UNIT_FLIGHT_IDENTITY,
        &commands,
    )
    .unwrap();
    let before_groups = sim.groups.clone();
    let before_cache = sim.command_package_state.clone();
    let target_row = sim.world.row_of(target).unwrap();
    sim.world.units.set_uid(
        target_row,
        sim.world.units.get_uid(target_row).wrapping_add(1),
    );
...
#[test]
fn exact_retail_unit_to_unit_flight_retargets_current_strafes_and_resumes() {
    let (mut sim, planes, target) = unit_unit_flight_fixture();
    let commands = retail_unit_unit_flight_commands();
    let target_row = sim.world.row_of(target).unwrap();
    let target_uid = sim.world.units.get_uid(target_row);
    let target_position = (
        sim.world.units.x_internal()[target_row],
    ...
    let receipt = sim
        .process_air_replay_batch(RETAIL_UNIT_UNIT_FLIGHT_IDENTITY, &commands)
        .unwrap();
    assert!(receipt.validates());
    assert_eq!(receipt.command_image, commands);
    ...
    assert!(receipt.air.is_empty());
    let flight = &receipt.flight_strafe[0];
    assert_eq!(flight.actors.len(), 13);
    assert!(flight.skipped_actors.is_empty());
    assert_eq!(flight.target.band, CanonicalObjectBand::Unit);
    assert_eq!(flight.target.address(), (3, 182));
    assert!(matches!(
        flight.target.generation,
        don_sim::systems::air_group_action_transaction::CanonicalObjectGeneration::Unit {
            id,
    ...
    for &plane in &planes {
        ...
        assert_eq!((strafe.target_who, strafe.target_o), (3, 182));
        assert_eq!(strafe.target_uid, target_uid);
        assert_eq!((strafe.xx, strafe.yy), target_position);
        assert_eq!(strafe.mandatory, 1);
        assert_eq!(strafe.air.returning, 0);
    }
...
#[test]
fn exact_retail_return_flight_rewrites_current_strafes_and_resumes() {
    let (mut sim, planes, target_row, stale_uid) = return_flight_fixture();
    let commands = retail_return_flight_commands();
    let before_rng = sim.world.random.state();

    let receipt = sim
        .process_air_replay_batch(RETAIL_RETURN_FLIGHT_IDENTITY, &commands)
        .unwrap();
    assert!(receipt.validates());
    assert_eq!(receipt.command_image, commands);
    assert_eq!(receipt.flight_strafe.len(), 1);
    ...
    assert!(receipt.air.is_empty());
    let flight = &receipt.flight_strafe[0];
    assert_eq!(flight.target.band, CanonicalObjectBand::Build);
    assert_eq!(flight.target.address(), (1, 2_008));
    assert_eq!(flight.target_uid, sim.builds[target_row].uid);
    assert_eq!(flight.target_position, sim.builds[target_row].position());
    assert_eq!(flight.actors.len(), 2);
    assert!(flight.skipped_actors.is_empty());
    assert!(flight
        .actors
        .iter()
    ...
    for &plane in &planes {
        ...
        assert_eq!(order.kind, OrderIndex::Strafe);
        assert_eq!(order.flags & ORDER_GROUP, ORDER_GROUP);
        assert_eq!((order.target_who, order.target_o), (-1, -1));
        assert_eq!(order.target_uid, stale_uid);
        assert_eq!((strafe.target_who, strafe.target_o), (-1, -1));
        assert_eq!(strafe.target_uid, stale_uid);
        assert_eq!((strafe.air.whose, strafe.air.oxx), (1, 2_008));
        assert_eq!(strafe.air.returning, 1);
        assert_eq!((strafe.xx, strafe.yy), (-1, -1));
        assert_eq!(strafe.mandatory, 1);
    }
...
#[test]
fn return_flight_stale_target_rolls_back_selection_and_every_actor() {
    let (mut sim, planes, target_row, _) = return_flight_fixture();
    let commands = retail_return_flight_commands();
    let players = std::array::from_fn(|play| (play == 0).then_some(1));
    let prepared = prepare_canonical_air_replay_batch(
        &sim.world,
        &sim.builds,
        &sim.groups,
    ...
    assert_eq!(
        ...
        Err(CanonicalAirPackageShellError::FlightStrafe(
            don_sim::systems::canonical_flight_strafe_host::CanonicalFlightStrafeError::ReturnTargetNotAuthoritative(
                don_sim::systems::air_group_action_transaction::CanonicalObjectIdentity {
                    owner: 1,
                    band: CanonicalObjectBand::Build,
                    o: 2_008,
                    generation: don_sim::systems::air_group_action_transaction::CanonicalObjectGeneration::BuildRow(
                        target_row as u32,
                    ),
                },
            ),
...
#[test]
fn return_flight_missing_actor_authority_and_exhausted_distance_branch_stay_red() {
    let (mut sim, _planes, _, _) = return_flight_fixture();
    sim.air_group_authority.units[0].return_flight_target = None;
    let before_world = sim.world.digest();
    let before_groups = sim.groups.clone();
    let before_cache = sim.command_package_state.clone();
    assert!(matches!(
        sim.process_air_replay_batch(RETAIL_RETURN_FLIGHT_IDENTITY, &retail_return_flight_commands()),
        Err(CanonicalAirPackageShellError::FlightStrafe(
            don_sim::systems::canonical_flight_strafe_host::CanonicalFlightStrafeError::MissingReturnTargetAuthority(_)
        )),
    ));
    assert_eq!(sim.world.digest(), before_world);
    assert_eq!(sim.groups.list, before_groups.list);
    ...
    let (mut sim, planes, _, _) = return_flight_fixture();
    let row = sim.world.row_of(planes[0]).unwrap();
    sim.world.units.mana_burn_mut()[row] = 1_000;
    let before_world = sim.world.digest();
    assert!(matches!(
        sim.process_air_replay_batch(RETAIL_RETURN_FLIGHT_IDENTITY, &retail_return_flight_commands()),
        Err(CanonicalAirPackageShellError::FlightStrafe(
            don_sim::systems::canonical_flight_strafe_host::CanonicalFlightStrafeError::ReturnActorFuelExhausted(_)
        )),
    ));
    assert_eq!(sim.world.digest(), before_world);

    let mut sim = build_flight_fixture();
    let mut commands = retail_build_flight_commands();
    commands[1][21..25].copy_from_slice(&1i32.to_le_bytes());
    let before_world = sim.world.digest();
    assert!(matches!(
        sim.process_air_replay_batch(RETAIL_BUILD_FLIGHT_IDENTITY, &commands),
        Err(CanonicalAirPackageShellError::Flight(
...
#[test]
fn exact_cached_queue_new_patrol_then_unit_flight_is_atomic_and_resumable() {
    let (mut sim, planes, target) = patrol_unit_flight_fixture();
    let commands = retail_patrol_unit_flight_commands();
    let target_row = sim.world.row_of(target).unwrap();
    let target_uid = sim.world.units.get_uid(target_row);
    let target_position = (
        sim.world.units.x_internal()[target_row],
    ...
    let receipt = sim
        .process_air_replay_batch(RETAIL_PATROL_UNIT_FLIGHT_IDENTITY, &commands)
        .unwrap();
    assert!(receipt.validates());
    assert_eq!(receipt.command_image, commands);
    assert_eq!(receipt.patrol.len(), 1);
    ...
    assert!(receipt.flight_strafe.is_empty());
    let patrol = &receipt.patrol[0];
    assert_eq!(patrol.request.queue, 2);
    assert_eq!(patrol.destination, (21_862, 58_426));
    assert_eq!(patrol.actors.len(), 9);
    assert!(patrol
        .actors
        .iter()
        .all(|actor| actor.after.kind == OrderIndex::AirPatrol));
    let flight = &receipt.flight_fresh_strafe[0];
    assert_eq!(flight.target.band, CanonicalObjectBand::Unit);
    assert_eq!(flight.target.address(), (4, 18));
    assert!(matches!(
        flight.target.generation,
        don_sim::systems::air_group_action_transaction::CanonicalObjectGeneration::Unit {
            id,
    ...
    for &plane in &planes {
        ...
        assert_eq!(order.kind, OrderIndex::Strafe);
        assert_eq!(order.flags & ORDER_GROUP, ORDER_GROUP);
        assert_eq!((strafe.target_who, strafe.target_o), (4, 18));
        assert_eq!(strafe.target_uid, target_uid);
        assert_eq!((strafe.xx, strafe.yy), target_position);
        assert_eq!(strafe.mandatory, 1);
        assert_eq!((strafe.air.oxx, strafe.air.whose), (-1, -1));
...
#[test]
fn changed_queue_new_unit_target_rolls_back_patrol_and_flight_together() {
    let (mut sim, planes, target) = patrol_unit_flight_fixture();
    let commands = retail_patrol_unit_flight_commands();
    let players = std::array::from_fn(|play| (play == 0).then_some(1));
    let map_tiles = (sim.map.world.xs, sim.map.world.ys);
    let prepared = prepare_canonical_air_replay_batch(
        ...
        &players,
        map_tiles,
        RETAIL_PATROL_UNIT_FLIGHT_IDENTITY,
        &commands,
    )
    .unwrap();
    let before_orders = planes
        ...
        .map(|&plane| sim.world.orders(sim.world.row_of(plane).unwrap()).clone())
        .collect::<Vec<_>>();
    let before_paths = planes
        .iter()
        .map(|&plane| sim.paths[sim.world.row_of(plane).unwrap()].clone())
        .collect::<Vec<_>>();
    let before_groups = sim.groups.clone();
    let before_cache = sim.command_package_state.clone();
    let target_row = sim.world.row_of(target).unwrap();
    ...
    for ((&plane, before_order), before_path) in planes.iter().zip(before_orders).zip(before_paths)
    {
        let row = sim.world.row_of(plane).unwrap();
...
#[test]
fn queue_first_patrol_flight_remains_outside_the_exact_cone() {
    let (mut sim, _, _) = patrol_unit_flight_fixture();
    let mut commands = retail_patrol_unit_flight_commands();
    commands[2][9] = 0;
    assert!(matches!(
        sim.process_air_replay_batch(RETAIL_PATROL_UNIT_FLIGHT_IDENTITY, &commands),
        Err(CanonicalAirPackageShellError::PatrolFlight(
            don_sim::systems::canonical_patrol_flight_host::CanonicalPatrolFlightError::UnsupportedPatrolQueue(0)
        ))
    ));
}
