...
fn measure(chunk: usize, mut work: impl FnMut(usize)) -> Run {
    ...
    Run { frames, dt: t0.elapsed().as_secs_f64() }
}
...
fn bench(label: &str, worlds: usize, units: usize, threads: usize, capacity: usize, per_frame: bool) {
    let cap = if capacity == 0 { don_sim::MAX_UNITS } else { capacity };
    ...
    row(label, worlds, units, threads, if per_frame { "step/frame" } else { "run" }, &r, b.digest());
}
...
fn single_world(units: usize) {
    let paths: [(&str, u8); 3] =
        [("shipped", 0u8), ("hand simd", 1), ("scalar branchy", 2)];
    for (pass, order) in [("pass A", [0usize, 1, 2]), ("pass B", [2usize, 1, 0])] {
        for i in order {
            ...
            row(&format!("{pass}, {name}"), 1, units, 1, "serial", &r, w.digest());
        }
    }
}
...
fn cross_check(worlds: usize, units: usize, frames: usize, threads: usize) {
    ...
    println!(
        "cross-check {worlds}w x {units}u over {frames} frames — serial {want:#018x}  \
         step/frame {:#018x}  run {:#018x}  lane-major {:#018x}   {}",
        per_frame.digest(),
        run.digest(),
        laneb.digest(),
        if per_frame.digest() == want && run.digest() == want && laneb.digest() == want {
            "ALL AGREE"
        } else {
            "*** DIVERGED ***"
        }
